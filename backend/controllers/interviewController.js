...
// Helper to construct the system prompt based on user details
const buildSystemPrompt = (role, experience_level, skills, interview_type, persona, resumeText, leetcodeData, pressureMode) => {
    ...
    const selectedPersona = personaGuides[persona] || "You are a standard professional corporate interviewer.";

    // LeetCode Profile Analysis Block
    let leetcodeAnalysis = "";
    if (leetcodeData) {
        const { profile, submitStats, tagProblemCounts } = leetcodeData;
        ...
        const strongest = allTags.slice(0, 3).map(t => t.tagName).join(", ");
        const weakest = allTags.slice(-3).map(t => t.tagName).join(", ");
...
3. **PHASE 3: Behavioral & Team Skills** - Conclude with soft-skill evaluation using the STAR method (Situation, Task, Action, Result). 

**MANDATORY**: You MUST explicitly announce when you are transitioning between phases (e.g., "Now that we've covered the algorithmic part, let's move on to Round 2: Technical Depth...").`;
...
6. **IF PRESSURE MODE IS ENABLED**: Occasionally (every 2-3 questions), throw a "curveball" or high-stress hypothetical scenario related to their previous answer. Push them to think on their feet.
---
...
export const startSession = async (req, res) => {
    try {
        const { role, experienceLevel, skills, interviewType, leetcodeUsername, githubUsername, persona, pressureMode, sessionMode } = req.body;
...
${ghProfile.repositories.map(repo => `- **${repo.name}** (${repo.language}): ${repo.description || 'No description provided.'}`).join('\n')}

**INSTRUCTION**: During PHASE 2 (Technical Depth), you MUST reference at least one of these projects and ask about a specific design decision, a challenge they faced, or how they might scale a particular feature.
---`;
            } catch (err) {
        ...
        if (leetcodeUsername) {
            leetcodeData = await fetchLeetCodeProfile(leetcodeUsername);
        }
        ...
        const systemInstruction = buildSystemPrompt(role, experienceLevel, skills, interviewType, persona, resumeText, leetcodeData, pressureMode);
    ...
    model: "gemini-3.1-flash-lite"
        ...
        const chat = model.startChat({
            history: [
                ...
                {
                    role: "model",
                    parts: [{ text: "Understood. I have reviewed your requirements and I am ready to conduct the interview. Please let me know when you are ready to start, or simply say hello." }],
                }
            ]
        });

        // Trigger the AI to send the first message (greeting + first question)
        const response = await chat.sendMessage('Hello, I am ready to begin the interview.');
        const reply = response.response.text();

        // Generate a simple in-memory session ID
        const sessionId = `session_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`;

        const initialMessages = [
            { role: "user", parts: [{ text: systemInstruction }] },
            { role: "model", parts: [{ text: "Understood. I have reviewed your requirements and I am ready to conduct the interview." }] },
            { role: "user", parts: [{ text: "Hello, I am ready to begin the interview." }] },
...
export const endSession = async (req, res) => {
    try {
        const { sessionId, fillerWordsCount } = req.body;
        const session = sessions.get(sessionId);
...
`;
        ...
        const response = await session.chat.sendMessage(finalPrompt);
        let reply = response.response.text();

        let scores = { overall: 0, communication: 0, technical: 0, confidence: 0, starMethod: 0 };
        try {
            const scoreMatch = reply.match(/SCORE_JSON:\s*(\{[\s\S]*?\})/);
            if (scoreMatch) {
                scores = JSON.parse(scoreMatch[1]);
                reply = reply.replace(/SCORE_JSON:\s*\{[\s\S]*?\}/, '').trim();
            }
        } catch (parseErr) {
            console.error("Failed to parse AI scores:", parseErr);
        }

        sessions.delete(sessionId);
        res.json({ finalReport: reply, scores });
    } catch (error) {
...
export const getLeetCodeProfile = async (req, res) => {
    try {
        ...
        if (!profile) {
            return res.status(404).json({ error: 'LeetCode profile not found or is private.' });
        }
        res.json(profile);
    } catch (error) {
        res.status(500).json({ error: error.message });
...
export const getGitHubProfileData = async (req, res) => {
    try {
        const { username } = req.params;
